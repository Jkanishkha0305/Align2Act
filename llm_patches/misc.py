...
from accessory.util.clip_grad import clip_grad_norm
...
def init_distributed_mode(args=SimpleNamespace()):
    ...
    if hasattr(args, 'available_dist_url'):
        torch.distributed.init_process_group(backend=args.dist_backend, init_method=args.available_dist_url,
    ...
    setup_for_distributed(args.rank % 8 == 0)

def init_distributed_mode_multi_url(args=SimpleNamespace()):
    ...
    if hasattr(args, 'available_dist_url'):
        ...
        raise Exception("Error to get available_dist_url")
    torch.distributed.barrier()
    # setup_for_distributed(args.rank == 0)
    setup_for_distributed(args.rank % 8 == 0)
